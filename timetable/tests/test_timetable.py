...
async def test_create_timetable(ac: AsyncClient, users_data: dict):
    data = {
        ...
        "from": "2024-10-24T18:00:00Z",
        "to": "2024-10-24T20:00:00Z",
        "room": "string"
    }
    statuses = {
        "admin": 200,
    ...
    assert response.status_code == statuses[users_data['username']], response.json()
...
async def test_create_appointments(ac: AsyncClient, time: str = "2024-10-24T12:00:00Z"):
    async with AsyncClient() as lac:
        response = await lac.post('http://localhost:8081/api/Authentication/SignIn', json={
            "username": 'user',
            "password": 'user'
...
async def test_delete_appointments(ac: AsyncClient, users_data: dict):
    appointments = await test_create_appointments(ac, "2024-10-24T12:30:00Z")
    statuses = {
        "admin": 200,
        "manager": 200,
        "doctor": 403,
